...
static constexpr Model dev15dd[] = {
    ...
    {Model::DetectRev, 0x0000, 0x0000, 0x0081, "AMD Ryzen Embedded V1807B with Radeon Vega"},
    {Model::DetectRev, 0x0000, 0x0000, 0x0082, "AMD Ryzen Embedded V1756B with Radeon Vega"},
    {Model::DetectRev, 0x0000, 0x0000, 0x0083, "AMD Ryzen Embedded V1605B with Radeon Vega"},
    {Model::DetectRev, 0x0000, 0x0000, 0x0085, "AMD Ryzen Embedded V1202B with Radeon Vega"},
    ...
    for (auto &device : devices) {
        if (device.dev == dev) {
            for (size_t j = 0; j < device.modelNum; j++) {
                ...
                if (model.mode & Model::DetectSub && (model.subven != subven || model.sub != sub)) continue;

                if (model.mode & Model::DetectRev && (model.rev != rev)) continue;
