...
void WRed::processKernel(KernelPatcher &patcher)
{
	...
	if (devInfo)
	{
		...
		{
			DBGLOG("wred", "non-apple-fw proceeding with devprops %d", graphicsDisplayPolicyMod);
			...
			DBGLOG("wred", "resulting applbkl value is %d", appleBacklightPatch);
...
size_t WRed::wrapFunctionReturnZero()
{
	return 0;
}
...
void WRed::processExternalProperties(IORegistryEntry *device, DeviceInfo *info, uint32_t vendor)
{
	...
	if (device->getProperty("preserve-names") == nullptr && currentExternalGfxIndex <= MaxExternalGfxIndex && (!name || strncmp(name, "GFX", strlen("GFX")) != 0))
	{
		char name[16];
...
void WRed::wrapFramebufferInit(IOFramebuffer *fb)
{
	...
	if (backCopy || zeroFill)
	{
		...
		{
			DBGLOG("wred", "fb info 1: %d:%d %u:%u:%u",
			...
			if (panels)
			{
				...
				for (auto &entry : appleBacklightData)
				{
					if (customName != nullptr && strcmp(customName, entry.deviceName) == 0)
					{
						...
						DBGLOG("wred", "using custom panel data for %s device", entry.deviceName);
					}
					else
					{
						...
						{
							SYSLOG("wred", "panel start cannot allocate %s data", entry.deviceName);
						}
					}
				}
...
bool WRed::getVideoArgument(DeviceInfo *info, const char *name, void *bootarg, int size)
{
