...
union Connector {
    ...
    static_assert(sizeof(LegacyConnector) == 16,
                  "LegacyConnector has wrong size");
    static_assert(sizeof(ModernConnector) == 24,
                  "ModernConnector has wrong size");
...
inline char *printConnector(char (&out)[N], T &connector) {
    snprintf(out, N,
             "type %08X (%s) flags %08X feat %04X pri %04X txmit %02X enc %02X "
             "hotplug %02X sense %02X",
             connector.type, printType(connector.type), connector.flags,
             connector.features, connector.priority, connector.transmitter,
             connector.encoder, connector.hotplug, connector.sense);
    return out;
}
 ...
 */
inline bool modern() { return getKernelVersion() >= KernelVersion::Sierra; }

/**
 *  Prints connectors
 *
 *  @param con  pointer to an array of legacy or modern connectors (depends on
 * the kernel version)
...
inline void print(Connector *con, uint8_t num) {
    ...
    for (uint8_t i = 0; con && i < num; i++) {
        char tmp[192];
        DBGLOG("con", "%u is %s", i,
               modern() ? printConnector(tmp, (&con->modern)[i])
...
inline bool valid(uint32_t size, uint8_t num) {
    return (size % sizeof(ModernConnector) == 0 &&
            size / sizeof(ModernConnector) == num) ||
           (size % sizeof(LegacyConnector) == 0 &&
            size / sizeof(LegacyConnector) == num);
}
 ...
 */
inline void copy(RADConnectors::Connector *out, uint8_t num,
                 const RADConnectors::Connector *in, uint32_t size) {
    bool outModern = modern();
    bool inModern = size % sizeof(ModernConnector) == 0 &&
                    size / sizeof(ModernConnector) == num;
